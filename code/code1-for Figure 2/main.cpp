...
void Solve1::computeServiceLevel_SeveralType() {
	...
	{
		cout << "查询的key" << this->currentSolutionOnShift.convertSolutionOnShiftToString() << endl;
		...
		cout << "存储的key" << this->currentSolutionOnShift.convertSolutionOnShiftToString() << endl;
	}
...
//				}
	...
	void show(){
		...
		cout << "当前解" << L.computeSumOfServiceTime(solve.currentSolutionOfServers) << endl;
	}
};
...
//int main_testUniform() 
{
	...
	Solve1 solve;
		...
		if (fmod(shiftsEnv.shifts[indexOfShift].startTime, onedayDivided) == 0 && fmod(shiftsEnv.shifts[indexOfShift].endTime, onedayDivided) == 1)
...
int main_youhua()
{
	...
	cout << "迭代次数" << countOfIter << endl;
	cout << "均匀化次数" << countUniformTimes << endl;
...
int main() {
	...
	if (Fanzhen_1_OrYouhua_0)main_fanzhen();
	else main_youhua();
			...
			for (int indexOfShifts = 1; indexOfShifts <= MaxNumOfShifts; indexOfShifts++)
				solutionOnShift.setServerOnShift(indexOfShifts,
					cplex.getValue(varNumOfServerInShift[indexOfShifts]));
						cout << "当前cplex目标值\t" << cplex.getValue(obj) << endl;
		}
	}
