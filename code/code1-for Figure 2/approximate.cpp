...
double APPROXIMATE::skellam(double serviceRate, double arrivalRate, int value){
	if (arrivalRate > maxValueOfArrivalRate || serviceRate > maxValueOfServiceRate) { cout << "error ahdsjkhakljh:skellam读取越界" << endl; system("pause"); return -1; }
...
double APPROXIMATE::valueOfBeta2_Right(int l, int z, double tao) {
	double theta = tao - double(int(tao));//todo
